...
def get_google_nasdaq_snippets(query="나스닥", count=10):
    chromedriver_path = "C:/tools/chromedriver-win32/chromedriver.exe"
    ...
    try:
        url = f"https://www.google.com/search?q={query}&hl=ko&gl=KR&ceid=KR:ko"
        driver.get(url)
        time.sleep(3)
        ...
        for el in elements[:count]:
            try:
                ...
                print(f"[나스닥 정보 파싱 오류] {e}")
                continue

        return results

    except Exception as e:
        print(f"[Google 나스닥 정보 크롤링 실패] {e}")
        return []
